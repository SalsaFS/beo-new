...
namespace App\Filament\Resources\BeoApprovals\Schemas;
...
class BeoApprovalForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->columns(1)
            ->components([
                Repeater::make('beoApprovals')
                    ->hiddenLabel()
                    ->saveRelationshipsUsing(function (Repeater $component, array $state) {
                        $record = $component->getRecord();
                        if (!$record) return;

                        $record->beoApprovals()->delete();

                        foreach ($state as $item) {
                            if (!empty($item['user_id'])) {
                                $record->beoApprovals()->create([
