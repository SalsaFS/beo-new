...
namespace App\Filament\Resources\BeoAmendmentApprovals\Schemas;
...
class BeoAmendmentApprovalForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->columns(1)
            ->components([
                Repeater::make('beoAmendmentApprovals')
                    ->hiddenLabel()
                    ->saveRelationshipsUsing(function (Repeater $component, array $state) {
                        $record = $component->getRecord();
                        if (!$record) return;

                        $record->beoAmendmentApprovals()->delete();

                        foreach ($state as $item) {
                            if (!empty($item['user_id'])) {
                                $record->beoAmendmentApprovals()->create([
