...
class BeoWeddingExport implements FromArray, WithTitle, ShouldAutoSize, WithStyles
{
    protected BeoWedding $beo;

    /** @var string[] */
    protected array $signaturePaths = [];

    public function __construct(BeoWedding $beo)
    {
        $this->beo = $beo->load([
            'client',
            'beoWeddingPackages.venue',
            'beoWeddingPackages.setup',
            'beoWeddingPackages.package',
            'beoWeddingFunctions.function',
            'beoWeddingFunctions.venue',
            'beoWeddingFunctions.setup',
            'beoWeddingFunctions.beoWeddingAdditionalMeals',
            'beoWeddingApprovals.user.position',
            'beoWeddingBreakdownPostings',
            'beoWeddingMakeUps.venue',
        ]);
    }
    ...
    {
        $beo = $this->beo;
        $dateIssued = \Carbon\Carbon::parse($beo->created_at)->format('d F Y');
        $dateOfFunction = \Carbon\Carbon::parse($beo->date_of_function)->format('d F Y');
        $thisYear = date('Y');

        $names = [];
        $venues = [];

        foreach ($beo->beoWeddingPackages as $package) {
        ...
        foreach ($beo->beoWeddingMakeUps as $makeUp) {
            $venues[] = $makeUp->venue->name;
        }
        $makeUpVenues = implode(' & ', $venues);

        $data = [
            ['BANQUET EVENT ORDER'],
            ['BEO.' . $thisYear, '', '', '', 'DATE ISSUED', '', $dateIssued, ''],
            ['EVENT NUMBER', '' . ((string) $beo->event_number ?? '-'), '', 'CLIENT NUMBER', '', ($beo->client->guest_number ?? '-'), '', ''],
            ['PIC', '' . ($beo->client->pic ?? '-'), '', 'TEL', '', ($beo->client->telephone ?? '-'), '', ''],
            ['ADDRESS', '' . ($beo->client->address ?? '-'), '', 'MOBILE', '', ($beo->client->mobile ?? '-'), '', '', '', '', '', ''],
            ['DAY/DATE/TIME OF FUNCTION', ($dateOfFunction ?? '-'), '', 'GUARANTEED', '', ($beo->guaranteed ?? '-') . ' persons', '', ''],
            ['PACKAGE', '' . ($packageNames ?? '-'), '', 'IN HOUSE CONTACT', '', ($beo->user->name ?? '-'), ''],
            ['VENUE', '' . ($packageVenues ?? '-'), '', 'MAKE UP ROOM', '', ($makeUpVenues ?? '-')],
            ['TIME', 'FUNCTION', '', 'VENUE', '', 'SET UP', '', 'PAX'],
        ];

        foreach ($beo->beoWeddingFunctions as $rundown) {
            $data[] = [
                ...
                $rundown->setup->name ?? '-',
                '',
                (string) $rundown->pax . ' Pax'
            ];
        }

        $data[] = ['SIGNED'];
        $data[] = ['"' . $beo->signed . '"'];
        $data[] = ['MENU', '', 'SET UP AND ARRANGEMENTS:', '', '', '', '', ''];

        $menuCellText = "";
        $additionalMeal = 0;

        foreach ($beo->beoWeddingFunctions as $function) {
            if (!empty($function->free_meal)) {
                $menuCellText .= "Free Stall " . $function->function->name . "\n";
                $menuCellText .= $function->free_meal . "\n";
                $menuCellText .= "\n";
            }

            if ($function->beoWeddingAdditionalMeals->isNotEmpty()) {
                $additionalMeal = 1;
                $menuCellText .= "Add Stall " . $function->function->name . "\n";
                foreach ($function->beoWeddingAdditionalMeals as $beoMenu) {
                    $menuCellText .= ($beoMenu->menu_name ?? '') . " " . (string) $beoMenu->pax . " pax\n";
                }
                $menuCellText .= "\n";
            }
        ...
        if (!empty($beo->menu_list)) {
            $menuListText = $this->cleanSetupArrangement($beo->menu_list);
            if (!empty($menuCellText)) {
                $menuCellText .= "\n\nMain Menu:\n" . $menuListText;
            } else {
                $menuCellText = $menuListText;
            }
        }
        ...
        $groupedBillings = $beo->beoWeddingBreakdownPostings->groupBy('revenue_type');
        ...
        foreach ($labels as $type => $headerLabel) {
            ...
            if ($items->isNotEmpty()) {
                ...
                $rightRows[] = ['Item', 'Rate', 'Amount', 'Total', 'Remark', ''];
                $totalPackage = 0;
                $totalFunction = 0;
                foreach ($items as $item) {
                    $total = ($item->amount ?? 0) * ($item->rate ?? 0);
                    $totalBreakdown = $totalBreakdown + $total;
                    $totalPackage += $total;
                ...
                if ($type === 'hotel') {
                    foreach ($beo->beoWeddingFunctions as $f) {
                ...
                $leftRows[] = [
                    $headerLabel,
                    'Rp ' . number_format($totalPackage, 0, ',', ',')
                ];
            }
            $rightRows[] = ['Total', '', '', 'Rp ' . number_format($totalBreakdown, 0, ',', ','), '', ''];
            $rightRows[] = ['', '', '', '', '', ''];

            if ($type === 'hotel' && $additionalMeal == 1) {
                $rightRows[] = ['ADDITIONAL MEAL', '', '', '', ''];
                $rightRows[] = ['Item', 'Rate', 'Pax', 'Total', 'Remark', ''];

                $totalAdditional = 0;
                foreach ($beo->beoWeddingFunctions as $function) {
                    if ($function->beoWeddingAdditionalMeals->isNotEmpty()) {
                        $totalAdditionalMeal = 0;
                        foreach ($function->beoWeddingAdditionalMeals as $add) {
                            $totalAdditionalMeal = $add->pax * $add->rate;
                            $totalAdditional = $totalAdditional + $totalAdditionalMeal;

                            $rightRows[] = [
                                ($add->beoWeddingFunction->function->name ?? '') . ' ' . ($add->menu_name ?? ''),
                                number_format($add->rate),
                                (string) $add->pax,
                                'Rp ' . number_format($totalAdditionalMeal),
                                $add->remark ?? '',
                                ''
                            ];
                        }
                ...
                $rightRows[] = ['Total', '', '', 'Rp ' . number_format($totalAdditional, 0, ',', ','), '', ''];
                $rightRows[] = ['', '', '', '', '', ''];
            }
        }

        $totalAll = $totalAll - ($beo->deposit ?? 0);
        $paymentNote = $this->cleanSetupArrangement($beo->payment_note ?? '');

        $leftRows[] = ['DEPOSIT', 'Rp ' . number_format($beo->deposit ?? 0, 0, ',', ',')];
        $leftRows[] = ['TOTAL', 'Rp ' . number_format($totalAll, 0, ',', ',')];
        $leftRows[] = ['PAYMENT NOTE : ', ''];
        $leftRows[] = [$paymentNote, ''];
        $leftRows[] = [$beo->payment_information ?? '', ''];
        $leftRows[] = ['', ''];
        ...
        $sortedApprovals = $beo->beoWeddingApprovals->sortBy('user.position.signature_positions');
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $totalRundown = $this->beo->beoWeddingFunctions->count();
        $startRow = 10;
        ...
        $internalRow = $startInternalRow + 1;
    ...
    {
        return 'BEO ' . $this->beo->client->pic;
    }
}
