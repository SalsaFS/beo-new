...
<style>
    ...
    .dark .billing-wrap .bl-c0 {
        background-color: #075985 !important;
        color: #e0f2fe !important;
    }

    .dark .billing-wrap .bl-c1 {
        background-color: #78350f !important;
        color: #fef3c7 !important;
    }

    .dark .billing-wrap .bl-c2 {
        background-color: #14532d !important;
        color: #dcfce7 !important;
    }

    .dark .billing-wrap .bl-c3 {
        background-color: #4c1d95 !important;
        color: #f3e8ff !important;
    }

    .dark .billing-wrap .bl-c4 {
        background-color: #831843 !important;
        color: #ffe4e6 !important;
    }

    .dark .billing-wrap .bl-c5 {
...
<div class="billing-wrap" style="display: flex; flex-direction: column; gap: 16px;">
    ...
    <div style="overflow: auto;">
        <table
            style="width: 100%; font-size: 13px; text-align: left; border-collapse: collapse; border: 1px solid #e5e7eb;">
            <tbody>
                @php
                ...
                @foreach ($labels as $type => $headerLabel)
                    ...
                    @if ($items->isNotEmpty())
                        <tr class="billing-header" style="background-color: #38bdf8;">
                            <td colspan="3" style="padding: 8px 16px; font-weight: 700; border: 1px solid #e5e7eb;">
                                {{ $headerLabel }}</td>
                        </tr>
                        ...
                        @foreach ($items as $item)
                            ...
                            <tr class="billing-row bl-c{{ $pkgIndex % count($colors) }}"
                                style="background-color: {{ $colors[$pkgIndex % count($colors)] }};">
                                <td colspan="3" style="padding: 8px 16px; font-style: italic; border: 1px solid #e5e7eb;">
                                    {{ $item->package->name }}</td>
                            </tr>
                            <tr class="billing-row">
                                <td style="padding: 8px 16px; border: 1px solid #e5e7eb;">{{ $item->pax }} pax</td>
                                <td style="padding: 8px 16px; border: 1px solid #e5e7eb;">Rp
                                    {{ number_format($ratePerPack, 0, ',', '.') }}</td>
                                <td style="padding: 8px 16px; border: 1px solid #e5e7eb;">Rp
                                    {{ number_format($unitRate, 0, ',', '.') }}</td>
                            </tr>
                        @endforeach
            ...
            <table
                ...
                <tbody>
                    ...
                    @foreach ($labels as $type => $headerLabel)
                        ...
                        @if ($items->isNotEmpty())
                            <tr class="billing-row bl-c{{ ($index - 1) % count($colors) }}"
                            ...
                            <tr class="billing-row">
                                <td colspan="3" style="padding: 8px 16px; font-style: italic; border: 1px solid #e5e7eb;">Additional
                                    Breakdown</td>
                            </tr>
                            ...
                            <tr class="billing-total" style="background-color: #f3f4f6; font-weight: 700;">
                                ...
                                <td style="padding: 8px 16px; border: 1px solid #e5e7eb;"></td>
                                <td style="padding: 8px 16px; border: 1px solid #e5e7eb;">Rp
                                    {{ number_format($subTotalType, 0, ',', '.') }}</td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
